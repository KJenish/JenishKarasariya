...
import {
  SiHtml5, SiCss3, SiJavascript, SiReact, SiNodedotjs, SiExpress, SiMongodb, SiGit, SiGithub,
  SiSass, SiTypescript, SiVuedotjs, SiAngular, SiPython, SiDjango,
  SiFlask, SiPostgresql, SiDocker, SiAmazonaws, SiMicrosoftazure, SiFirebase
} from 'react-icons/si';
      ...
      <div style={{ height: "90vh", paddingTop: "50px", marginBottom: "50px" }} >
        <div className="experience-container" >
          ...
          <p className="experience-description">
            I've been crafting web solutions for several years, and I continue to hone my skills and keep up with industry trends.
          </p>
          <div className="skills-grid">
            {skills.map((skill, index) => {
              const { Icon } = skill; // Destructure Icon here
              return (
                <div key={index} className="skill-item" style={{ color: skill.color }}>
                  <Icon size="3em" />
                  <h4>{skill.name}</h4>
                </div>
              );
  ...
  <div key={index} className={`timeline-item ${item.side}`}>
    ...
    {item.side === 'left' && (
      <div className="timeline-icon-container">
        {item.type === 'education' ? <FaGraduationCap className="timeline-icon" /> : <FaBriefcase className="timeline-icon" />}
      </div>
    )}
    {item.side === 'right' && (
      <div className="timeline-icon-container">
